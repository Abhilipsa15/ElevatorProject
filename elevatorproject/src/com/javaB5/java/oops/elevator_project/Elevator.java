...
public class Elevator {
	...
	private Map<Integer, List<Integer>> requestedPathMap;
	...
	public Elevator() {
		...
		this.requestedPathMap = new HashMap<>();
	...
	public Map<Integer , List<Integer>> getRequestedPathMap(){
...
//			callElevator(random.nextInt(11), random.nextInt(10) + 1);
	...
	// TO DO #1
		public void callElevator(int start, int destination) {
			...
			if (requestedPathMap.containsKey(start))// if already START is in map, add the destination in the list
				requestedPathMap.get(start).add(destination);
			else {// else add the new key aas START with the list containing our DESTINATION

//				 requestedPathsMap.put(start, new ArrayList<>() {
				requestedPathMap.put(start, new ArrayList() {
					{
						add(destination);
					}
				});
		...
		private void processFloor(int floor) throws InterruptedException {
			...
			if (requestedPathMap.containsKey(floor)) {// Onboarding people and their destination
				System.out.println("BOARDING at Floor : " + floor);
				requestedPathMap.get(floor).forEach(destinationFloor -> currentFloorDestinations[destinationFloor] = true);// Marked
																															// true
																															// for
																															// next
																															// traversals
				requestedPathMap.remove(floor);// removing the entry from map as we have marked all the destination
			}
		...
		private void moveElevator() throws InterruptedException {
			...
			if (!Arrays.asList(currentFloorDestinations).contains(true) && requestedPathMap.isEmpty()) {// STOPPING THE
																											// ELEVATOR
																											// :checking
																											// destinations
																											// are reached
